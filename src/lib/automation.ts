...
export async function executeCommand(
  ...
  if (command.mode === "deep-link") {
    await openCodexUrl(command.value);
    return undefined;
  }
  if (command.mode === "mode-toggle") {
    return toggleLiveMode(command.value as CodexMode);
  }
  if (threadId) {
    await openThread(threadId);
  }
  await runControl(command.mode, command.value);
  return undefined;
}
...
export async function openThread(threadId: string): Promise<void> {
  await openCodexUrl(threadDeepLink(threadId));
}

export async function openNewChat(path?: string): Promise<void> {
  await openCodexUrl(newChatDeepLink(path ? { path } : {}));
}
...
export async function launchWorkflow(
  ...
  await openCodexUrl(
    newChatDeepLink({ prompt: workflow.prompt, ...(path ? { path } : {}) }),
  );
}

export async function openSkills(): Promise<void> {
  await openCodexUrl(codexDeepLink("skills"));
}

export async function applyReasoning(level: string): Promise<LivePickerState> {
  return applyLiveReasoning(level);
}
...
export async function applyModel(slug: string): Promise<LivePickerState> {
  return applyLiveModel(slug);
}
