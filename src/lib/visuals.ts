...
function wordmarkGraphic(icon: "yeet" | "yolo", accent: string): string {
  ...
  return `<g>
    <svg x="18" y="18" width="108" height="58" viewBox="${wordmark.viewBox}" preserveAspectRatio="xMidYMid meet" overflow="visible">
      <path d="${wordmark.d}" fill="#05070A" opacity=".75" transform="translate(2.2 3.2)"/>
      <path d="${wordmark.d}" fill="#F7F9FC"/>
    </svg>
...
export function commandKeySvg(
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144" viewBox="0 0 144 144">
    ...
    ${stateMarkup}
    ${glyph}
    <text x="72" y="121" text-anchor="middle" fill="#FFFFFF" font-family="-apple-system,system-ui,sans-serif" font-size="16" font-weight="750">${escapeXml(first)}</text>
    ${second ? `<text x="72" y="132" text-anchor="middle" fill="#AAB3BF" font-family="-apple-system,system-ui,sans-serif" font-size="10" font-weight="700" letter-spacing=".35">${escapeXml(second)}</text>` : ""}
  </svg>`;
}
...
export function keycapSvg(
  ...
  const subtitle =
    description.length > 14
      ? `${description.slice(0, 13)}…`.toUpperCase()
  ...
  const caption = wordmark
    ? `<text x="72" y="125" text-anchor="middle" fill="#D5DAE2" font-family="-apple-system,system-ui,sans-serif" font-size="12" font-weight="750" letter-spacing=".45">${escapeXml(subtitle)}</text>`
    : subtitle
      ? `<text x="72" y="109" text-anchor="middle" fill="#FFFFFF" font-family="-apple-system,system-ui,sans-serif" font-size="15" font-weight="750">${escapeXml(title)}</text>
         <text x="72" y="126" text-anchor="middle" fill="#8D97A5" font-family="-apple-system,system-ui,sans-serif" font-size="9" font-weight="700" letter-spacing=".4">${escapeXml(subtitle)}</text>`
      : `<text x="72" y="122" text-anchor="middle" fill="#FFFFFF" font-family="-apple-system,system-ui,sans-serif" font-size="16" font-weight="750">${escapeXml(title)}</text>`;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144" viewBox="0 0 144 144">
    <rect width="144" height="144" fill="#090B0F"/>
    ${glyph}
    ${caption}
