...
import { applyModel, openModelMenu } from "../lib/automation.js";
...
export class ModelAction extends SingletonAction<ModelSettings> {
  ...
  async onTouchTap(event: TouchTapEvent<ModelSettings>): Promise<void> {
  ...
  private async applyPreview(
    ...
    try {
      ...
      const live = await applyModel(option.slug);
  ...
  private async draw(
    ...
    await actionInstance.setFeedback(
      modelFeedback(state, codexStore.modelSnapshot().options),
    );
  }
}
