...
const rasterizer = process.env.RSVG_CONVERT ?? "rsvg-convert";
const imageMagick = process.env.MAGICK ?? "magick";
...
for (const { id: pageId, slug } of contract.pages) {
  ...
  for (let row = 0; row < 2; row += 1) {
    for (let column = 0; column < 4; column += 1) {
      ...
      const colors144 = Number(
        run(imageMagick, [png144, "-format", "%k", "info:"]).trim(),
      );
      const colors72 = Number(
        run(imageMagick, [png72, "-format", "%k", "info:"]).trim(),
      );
      if (
        !Number.isFinite(colors144) ||
        !Number.isFinite(colors72) ||
        colors144 < 8 ||
        colors72 < 8
      ) {
...
const report = {
  ...
  rasterArtifactCount: renderedKeys.length * 2,
  minimumColorsAt144: Math.min(
    ...renderedKeys.map(({ colors144 }) => colors144),
  ),
...
function renderAction(action, index) {
  ...
  switch (action.UUID) {
    ...
    case "com.todd.streamdeckcodex.approval-mode":
      return visuals.approvalKeySvg(settings.mode ?? "custom");
