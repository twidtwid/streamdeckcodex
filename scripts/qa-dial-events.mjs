...
function native(action, value) {
  const result = spawnSync(nativeControl, [action, ...(value ? [value] : [])], {
    encoding: "utf8",
  });
...
const initialPlan = native("mode-read", "plan").active;
if (initialPlan) {
...
try {
  ...
  if (!modelActive || !reasoningActive) {
    throw new Error("The dials did not emit verified ACTIVE feedback");
  }

  const commandAction = "com.todd.streamdeckcodex.command";
  const planContext = "qa-plan-command";
  const planSettings = { commandIndex: 8 };
  socket.send(
    JSON.stringify(
  ...
  server.close();
  const currentPlan = native("mode-read", "plan").active;
