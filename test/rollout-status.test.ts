...
describe("parseRolloutLines", () => {
  ...
  it("caches parsed inputs without caching acknowledgement or time conclusions", () => {
    const events = parseRolloutEvents(
      [
        JSON.stringify(event(1, "event_msg", "task_started")),
        JSON.stringify(event(2, "event_msg", "task_complete")),
      ].join("\n"),
    );
    const completedAt = Date.parse(at(2));

    expect(reduceRolloutEvents(events, { now: completedAt }).status).toBe(
      "unread",
    );
    expect(
      reduceRolloutEvents(events, {
        now: completedAt,
        acknowledgedAt: completedAt,
      }).status,
    ).toBe("idle");

    const active = parseRolloutEvents(
      JSON.stringify(event(1, "event_msg", "task_started")),
    );
    expect(reduceRolloutEvents(active, { now: Date.parse(at(2)) }).status).toBe(
      "thinking",
    );
    expect(
      reduceRolloutEvents(active, {
        now: Date.parse(at(1)) + 31 * 60 * 1000,
      }).status,
    ).toBe("idle");
  });
});
